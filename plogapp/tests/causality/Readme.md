The programs in this directory, adapted to the syntax of the solver is a product of joint research with Dr.Michael Gelfond on causal analysis in trajectories of dynamic domains. Each program represents T(S) for some scenario S and background theory T. Some axioms whose bodies are false in every model of all the presented scenarios are omitted to shorted the presentation. Note that  our solver uses cardinality-based preference for consistency restoring rules. 
